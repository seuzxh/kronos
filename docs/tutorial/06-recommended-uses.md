...
> 基于论文实际验证、专家批判性分析、以及我们**四次实验**的一手教训。
>
...
本指南曾在实验 3 后把"A 股选股(特征提取器路线)"列为🟢推荐,基于 RankIC +6.4%。**这是错的**。

实验 4 补充完整回测后证实:Kronos 特征**拖累了超额收益**(年化 -16% ~ -65%),RankIC 提升不等于赚钱。**A 股选股现已全部改为 🔴 不推荐**(无论端到端还是特征提取器)。

最终结论:**Kronos 对 A 股横截面选股没有实际 alpha**(四次实验一致)。它在其他场景(合成数据、波动率、冷启动)可能仍有效,但选股这条路已盖棺定论。详见 [实验 4 纠错报告](../finetune/2026-07-kronos-selected-winsorize/ablation-report.md)。

---

## TL;DR

Kronos 是一个**优秀的"K 线形态预测器"**。**端到端选股不行,但作为特征提取器有效**(实验 3 验证)。它的价值在不同场景下差异巨大:
...
| 🔴 **A 股横截面选股** | **不推荐** | **四次实验均失败**(端到端 RankIC≈0,特征提取器超额 -16%~-65%)|
| 🔴 高频 tick 级策略 | 不推荐 | 用 LOBERT 等订单簿模型更合适 |

> 💡 **本表格已被实验 4 纠正**(2026-07-21):此前曾把"A 股选股(特征提取器)"单列为🟢推荐,基于实验 3 的 RankIC +6.4%。实验 4 补充完整回测后发现超额收益实际下降,A 股选股已统一改为 🔴 不推荐。
...
> ✅ **实验 3 已验证此路线有效**(2026-07-21)!我们用 5min Kronos 提取 8 个特征注入 LGBM,OOS RankIC 提升 6.4%。关键发现:Kronos 特征本质是**反转信号**,与 LGBM 原有的动量因子互补。详见 [07 实验 3 突破](07-breakthrough-features.md) 和 [实验 3 完整档案](../finetune/2026-07-kronos-5min-features-lgbm/README.md)。
>
> **重要前提**:必须微调。预训练 base 版有 A 股看跌偏差,直接用反而拖累 LGBM(RankIC -28%);微调版校准了偏差才有效。
