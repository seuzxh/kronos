...
> 基于论文实际验证、专家批判性分析、以及我们**三次实验**的一手教训(2 次端到端选股失败 + 1 次特征提取器成功)。
>
...
| 🟢 **A 股选股(特征提取器路线)** | **推荐(已验证)** | **实验 3**:Kronos 特征 → LGBM,RankIC +6.4%。必须微调 |
...
> 💡 **重要更新(2026-07-21)**:本指南最初基于实验 1、2 的失败,把"A 股选股"列为不推荐。**实验 3 通过特征提取器路线成功**(RankIC +6.4%),证明问题不在 Kronos 本身,而在"怎么用"。详见 [07 实验 3 突破](07-breakthrough-features.md)。
