...
from data_loader import apply_price_limits, load_csv, load_qlib
...
@app.post("/api/predict-qlib")
async def predict_qlib(
    symbol: str = Form(...),
    start_time: str = Form("2024-01-01"),
    end_time: str = Form("2025-05-16"),
    lookback: Optional[int] = Form(None),
    pred_len: Optional[int] = Form(None),
    temperature: Optional[float] = Form(None),
    top_p: Optional[float] = Form(None),
    ...
    try:
        df = load_qlib(
            ...
            end_time=end_time,
            provider_uri=_config.qlib_provider_uri,
        )
    except ImportError:
        raise HTTPException(
            status_code=503,
            detail="pyqlib is not installed. Install with: pip install pyqlib",
        )
    ...
    if len(df) < effective_lookback:
        raise HTTPException(
            status_code=400,
            detail=f"Qlib returned {len(df)} rows for {symbol}, need at least {effective_lookback}",
        )
    ...
    if _llm_analyzer is not None and _llm_analyzer.is_available():
        analysis = _llm_analyzer.analyze_prediction(
            pred_df, last_close, symbol=symbol,
        )
